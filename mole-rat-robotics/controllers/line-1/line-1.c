...
int main() {
  ...
  for (;;) {  // Main loop
    ...
    // Check for line on left or right and adjust speed to turn away
    if (gs_value[GS_LEFT] < GS_WHITE) {
      speed[RIGHT] = 0;
    } else if (gs_value[GS_RIGHT] < GS_WHITE) {
      speed[LEFT] = 0;
    }

    /* Debugging printing
    
    printf("Sensors Left %d Right %d  Speed Left %d Right %d  \n", 
      gs_value[GS_LEFT], gs_value[GS_RIGHT],
